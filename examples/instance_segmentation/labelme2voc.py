...
def main():
    ...
    for filename in sorted(glob.glob(osp.join(args.input_dir, "*.json"))):
        ...
        label_file = labelfuse.LabelFile(filename=filename)
        ...
        img = labelfuse.utils.img_data_to_arr(label_file.imageData)
        imgviz.io.imsave(out_img_file, img)

        cls, ins = labelfuse.utils.shapes_to_label(
            img_shape=img.shape,
        ...
        # class label
        labelfuse.utils.lblsave(out_clsp_file, cls)
        if not args.nonpy:
        ...
        if not args.noobject:
            # instance label
            labelfuse.utils.lblsave(out_insp_file, ins)
            if not args.nonpy:
